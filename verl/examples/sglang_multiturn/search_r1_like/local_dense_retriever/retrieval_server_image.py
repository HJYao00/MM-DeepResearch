...
class Encoder:
    ...
    def encode(self, query_list: list[str], is_query=True) -> np.ndarray:
        ...
        if 'jina' in self.model_name.lower():
            ...
            query_emb = torch.stack(query_emb, dim=0)
            # print(query_emb, len(query_emb))
...
class BM25Retriever(BaseRetriever):
    ...
    def _search(self, query: str, num: int = None, return_score: bool = False):
        ...
        if self.contain_doc:
            results = []
            for hit in hits:
                # 1. 获取并解析原始 JSON (只做一次 IO 和 JSON 解析，效率更高)
                raw_json = self.searcher.doc(hit.docid).raw()
                doc_data = json.loads(raw_json)
                
                # 2. 提取 contents 和 images
                content = doc_data.get("contents", "")
                # 使用 .get() 防止某些数据没有 images 字段导致报错
                images = doc_data.get("img_path", []) 
                ...
                # 4. 组装结果字典，将 images 加入其中
                results.append({
                    "title": title,
                    "text": text,
                    "contents": content,
                    "img_path": images  # <--- 这里实现了联合返回
                })
        else:
            # 如果不包含文档，需要从 corpus 加载 (注意：这里也需要确保 load_docs 能加载 images)
            # 假设你的 load_docs 仅仅是从 jsonl 读取行，通常不需要改动，
            # 除非 load_docs 内部有过滤字段的逻辑。
...
class MultiModalRetriever(BaseRetriever):
    ...
    def _search(self, query: str, num: int = None, return_score: bool = False):
        ...
        # print('397', query_emb)
        # scores, idxs = self.index_dict['image'].search(query_emb, k=num)
    ...
    def _batch_search(self, query_list: list[str], num: int = None, return_score: bool = False):
        ...
        for start_idx in tqdm(range(0, len(query_list), self.batch_size), desc="Retrieval process: "):
            ...
            # batch_scores, batch_idxs = self.index_dict['image'].search(batch_emb, k=num)
...
def get_retriever(config):
    ...
    elif 'jina' in config.retrieval_method:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index_path", type=str, default="inews/e5_Flat.index", help="Corpus indexing file."
    )
    parser.add_argument(
        "--corpus_path",
        type=str,
        default="inews/category_news_education_2025_process.jsonl",
