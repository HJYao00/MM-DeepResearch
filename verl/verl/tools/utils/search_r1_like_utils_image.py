...
def _passages2string(retrieval_result, query):
    ...
    for doc_item in retrieval_result:
        ...
        idx += 1
    return format_reference.strip(), idx
        ...
        try:
            ...
            if raw_results:
                ...
                for retrieval, query in zip(raw_results, query_list):
                    # formatted, valid_response = _passages2string(retrieval, query)
                    # pretty_results.append(formatted)
                    # # total_results += len(retrieval) if isinstance(retrieval, list) else 1
                    # total_results += valid_response

                    for doc_item in retrieval:
                        if doc_item['score'] < score_threshold:
                            continue
                        img_path = doc_item['document']['img_path']
                        title = doc_item['document']['text']
                        result_image.append(img_path)
                        result_title.append(title)
                        total_results += 1

                # final_result = 'The image search results have been successfully returned; please perform inference based on the retrieved images.'
                ...
                # print('inside result_image', result_image)
                if total_results == 0:
