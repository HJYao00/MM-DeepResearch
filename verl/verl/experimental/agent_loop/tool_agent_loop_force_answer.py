...
class AgentState(Enum):
    ...
    FORCE_ANSWERING = "force_answering"
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        while state != AgentState.TERMINATED:
            ...
            elif state == AgentState.FORCE_ANSWERING:
                state = await self._handle_force_answering_state(agent_data, sampling_params)
            else:
    ...
    async def _handle_force_answering_state(self, agent_data: AgentData, sampling_params: dict[str, Any]) -> AgentState:
        ...
        print('agent_data.prompt_ids', agent_data.prompt_ids)
        print('output.token_ids', output.token_ids)
    ...
    async def _handle_generating_state(
        ...
        if (not ignore_termination and len(agent_data.response_mask) + len(output.token_ids) >= self.response_length) or \
            (self.max_assistant_turns and agent_data.assistant_turns >= self.max_assistant_turns) or \
            (self.max_user_turns and agent_data.user_turns >= self.max_user_turns):
            # 如果没有answer在里面，插入answer，强制输出
            output_token = self.tokenizer.decode(output.token_ids, skip_special_tokens=True)
            if '<answer>' not in output_token:
                ## 去掉output.token_ids (list) 末尾的151645，可能包含多个
                def remove_trailing_target(lst, target=151645):
                    while lst and lst[-1] == target:
                        lst.pop()
                    return lst
                output.token_ids = remove_trailing_target(output.token_ids)
                output.token_ids += [27, 9217, 397]
                ## prompt ids加上<answer>\n
                agent_data.response_ids = output.token_ids
                agent_data.prompt_ids = agent_data.prompt_ids + output.token_ids
                agent_data.response_mask += [1] * len(agent_data.response_ids)
                if output.log_probs:
                    agent_data.response_logprobs += output.log_probs
                return AgentState.FORCE_ANSWERING
        agent_data.response_ids = output.token_ids
        agent_data.prompt_ids += agent_data.response_ids
        agent_data.response_mask += [1] * len(agent_data.response_ids)
        if output.log_probs:
        ...
        if not ignore_termination and len(agent_data.response_mask) >= self.response_length:
            output_token = self.tokenizer.decode(output.token_ids, skip_special_tokens=True)
            if '<answer>' not in output_token:
                print('here terminated??')
                print('len(agent_data.response_mask)', len(agent_data.response_mask), 'self.response_length', self.response_length)
            return AgentState.TERMINATED
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        if len(agent_data.response_mask) + len(response_ids) >= self.response_length:
            print('tool response terminated!!!!!!')
            print('tool agent_data.prompt_ids', agent_data.prompt_ids)
            ## 去掉output.token_ids (list) 末尾的151645，可能包含多个
            # def remove_trailing_target(lst, target=151645):
            #     while lst and lst[-1] == target:
            #         lst.pop()
            ...
            # output.token_ids += [27, 9217, 397]
            ## prompt ids加上<answer>\n
            agent_data.response_ids += [27, 9217, 397]
            agent_data.prompt_ids = agent_data.prompt_ids + [27, 9217, 397]
            agent_data.response_mask += [1] * len(agent_data.response_ids)
            # if output.log_probs:
            #     agent_data.response_logprobs += output.log_probs
            return AgentState.FORCE_ANSWERING
